...
class CriticalRouteTests(unittest.TestCase):
    ...
    def test_uploads_presign_put(self):
        ...
        out = uploads.presign(body=body, s3=fake_s3, current_user=current_user)
    ...
    def test_analyze_start_rejects_foreign_upload_key(self):
        ...
        with self.assertRaises(HTTPException) as ctx:
            analyze_job.start_analyze(
                body=analyze_job.StartAnalyzeRequest(
                    s3_key=f"users/{user.id + 1}/uploads/study-1.png",
                    report_text=None,
                ),
                current_user=user,
            )
    ...
    def test_jobs_get_job_hides_foreign_job(self):
        ...
        with patch("app.api.routes.jobs.get_session", side_effect=self.SessionLocal):
            with self.assertRaises(HTTPException) as ctx:
                jobs.get_job(job_id="job-private", current_user=other)
    ...
    def test_studies_get_study_not_found(self):
        ...
        try:
            fake_s3 = Mock()
            with self.assertRaises(HTTPException) as ctx:
                studies.get_study(study_id=9999, session=session, s3=fake_s3, current_user=user)
        finally:
            session.close()
    ...
    def test_studies_get_study_hides_foreign_study(self):
        owner = self._create_user(email="owner@example.com")
        other = self._create_user(email="other@example.com")
        session = self.SessionLocal()
        try:
            ...
            fake_s3 = Mock()
            with self.assertRaises(HTTPException) as ctx:
                studies.get_study(study_id=study_id, session=session, s3=fake_s3, current_user=other)
        finally:
            session.close()
