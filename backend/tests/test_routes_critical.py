...
class CriticalRouteTests(unittest.TestCase):
    ...
    def test_knowledge_routes_require_authentication(self):
        self.assertTrue(knowledge.router.dependencies)
    ...
    def test_uploads_presign_put(self):
        ...
        out = uploads.presign(body=body, s3=fake_s3, current_user=user)

        self.assertEqual(out.method, "PUT")
        self.assertEqual(out.url, "https://signed.example/upload-put")
        self.assertIsNone(out.fields)
        self.assertTrue(out.key.startswith(f"users/{user.id}/uploads/"))
    ...
    def test_analyze_start_creates_job_and_dispatches_task(self):
        user = self._create_user()
        s3_key = f"users/{user.id}/uploads/study-1.png"
        with (
        ...
        try:
            ...
            self.assertEqual(job.s3_key, s3_key)
        finally:
            session.close()
    ...
    def test_analyze_start_rejects_foreign_upload_key(self):
        user = self._create_user()

        with self.assertRaises(HTTPException) as ctx:
            analyze_job.start_analyze(
                body=analyze_job.StartAnalyzeRequest(
                    s3_key="users/999/uploads/study-1.png",
                    report_text=None,
                ),
                current_user=user,
            )
    ...
    def test_jobs_get_job_hides_foreign_jobs(self):
        ...
        try:
            session.add(
                Job(
                    ...
                    status="completed",
                    progress=100,
                    s3_key=f"users/{owner.id}/uploads/study-1.png",
                    result={"study_id": 1},
                )
            )
            session.commit()
        ...
        with patch("app.api.routes.jobs.get_session", side_effect=self.SessionLocal):
            with self.assertRaises(HTTPException) as ctx:
                jobs.get_job(job_id="job-foreign", current_user=intruder)
    ...
    def test_jobs_get_job_not_found(self):
        ...
        with patch("app.api.routes.jobs.get_session", side_effect=self.SessionLocal):
            with self.assertRaises(HTTPException) as ctx:
                jobs.get_job(job_id="missing-job", current_user=user)
        self.assertEqual(ctx.exception.status_code, 404)
    ...
    def test_studies_get_study_maps_findings_and_signed_url(self):
        ...
        try:
            study = Study(
                ...
                patient_id="P-001",
                modality="XR",
                image_s3_key=f"users/{user.id}/uploads/xray.png",
            )
            session.add(study)
            session.commit()
            ...
            out = studies.get_study(study_id=study.id, session=session, s3=fake_s3, current_user=user)
        finally:
            session.close()
        ...
        fake_s3.generate_presigned_get.assert_called_once_with(f"users/{user.id}/uploads/xray.png")
    ...
    def test_studies_get_study_hides_foreign_studies(self):
        ...
        try:
            study = Study(
                ...
                patient_id="P-001",
                modality="XR",
                image_s3_key=f"users/{owner.id}/uploads/xray.png",
            )
            session.add(study)
            session.commit()
            session.refresh(study)
            ...
            with self.assertRaises(HTTPException) as ctx:
                studies.get_study(study_id=study.id, session=session, s3=Mock(), current_user=intruder)
        finally:
            session.close()
    ...
    def test_studies_get_study_not_found(self):
        ...
        try:
            with self.assertRaises(HTTPException) as ctx:
                studies.get_study(study_id=9999, session=session, s3=Mock(), current_user=user)
        finally:
            session.close()
