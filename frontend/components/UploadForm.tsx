...
export default function UploadForm({ onAnalysisComplete }: UploadFormProps) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 4) Poll for completion with the Authorization header.
  ...
  useEffect(() => {
    if (result?.status === "completed") {
      (async () => {
        try {
          const token = localStorage.getItem("token");
          const studyId = result?.result?.study_id;
          if (!studyId) {
            throw new Error("Analysis completed without a study result");
          }
          const studyResp = await fetch(
            `${backendUrl}/api/studies/${studyId}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            }
          );
